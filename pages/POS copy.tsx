...
export const POS: React.FC = () => {
  ...
  const handleFacebookCallback = () => {
    // Check if URL contains Facebook login callback parameters
    const urlParams = new URLSearchParams(window.location.hash.substring(1));
    const token = urlParams.get("token");
    const pageId = urlParams.get("pageId");

    if (token && pageId) {
      // Store Facebook token and pageId in localStorage for messenger usage
      localStorage.setItem("facebookToken", token);
      localStorage.setItem("facebookPageId", pageId);

      // Show success message
      toast.success(
        "Facebook login successful! Messenger features are now enabled.",
      );

      // Clear the URL parameters without page reload
      window.history.replaceState(null, "", window.location.pathname);

      console.log("Facebook token stored:", token);
      console.log("Facebook page ID stored:", pageId);
    }
  };
